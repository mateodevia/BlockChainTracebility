...
function OrginGraphViz() {
  ...
  useEffect(() => {
    fetch("/v1/api/trus/id/3668dc40-7457-11ea-abce-1b474a67a6e2-0/origen").then(
      (response) => {
    ...
    <div>
      ...
      <svg className="lineas lineaFinal">
        <line
          ...
          y2="50"
          style={{
            stroke: colores[buscado?.dueños[buscado?.dueños.length - 1]],
          }}
        />
      </svg>
      <div
        ...
        style={{
          backgroundColor:
            coloresClaros[buscado?.dueños[buscado?.dueños.length - 1]],
          borderColor: colores[buscado?.dueños[buscado?.dueños.length - 1]],
        }}
      ></div>
